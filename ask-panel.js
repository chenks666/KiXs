...
(function (global) {
  ...
  const AVATAR_WEST = `
    <svg viewBox="0 0 100 100" style="width:100%;height:100%;display:block;">
      <defs>
        <radialGradient id="aw-bg" cx="50%" cy="38%"><stop offset="0%" stop-color="#4a2a7a"/><stop offset="100%" stop-color="#1a0f33"/></radialGradient>
      </defs>
      ...
      <!-- 披肩长发（紫） -->
      <path d="M34 52 Q26 40 34 26 Q38 17 50 16 Q62 17 66 26 Q74 40 66 52 Q68 74 56 77 Q50 79 44 77 Q32 74 34 52Z" fill="#6a4a9a"/>
      <path d="M40 60 Q38 72 44 76" stroke="#5a3a8a" fill="none" stroke-width="3" stroke-linecap="round"/>
      <path d="M60 60 Q62 72 56 76" stroke="#5a3a8a" fill="none" stroke-width="3" stroke-linecap="round"/>
      <!-- 刘海 -->
      <path d="M36 40 Q38 26 50 25 Q62 26 64 40 Q58 33 50 32 Q42 33 36 40Z" fill="#5a3a8a"/>
      <!-- 脸 -->
      <ellipse cx="50" cy="47" rx="13" ry="14" fill="#f2c9a0"/>
      <!-- 闭眼微笑（弯月眼 + 长睫毛） -->
      <path d="M42 46 Q45 49.5 48 46" stroke="#4a2a4a" fill="none" stroke-width="1.5" stroke-linecap="round"/>
      <path d="M52 46 Q55 49.5 58 46" stroke="#4a2a4a" fill="none" stroke-width="1.5" stroke-linecap="round"/>
      <path d="M41.5 44.5 L44 46.5 M59 44.5 L56 46.5" stroke="#4a2a4a" stroke-width="1" stroke-linecap="round" opacity=".7"/>
      <path d="M44 53 Q50 57 56 53" stroke="#8a5a3a" fill="none" stroke-width="1.1" stroke-linecap="round"/>
      <!-- 腮红 -->
      <ellipse cx="42" cy="51" rx="2.7" ry="1.5" fill="#e88a8a" opacity=".45"/>
      <ellipse cx="58" cy="51" rx="2.7" ry="1.5" fill="#e88a8a" opacity=".45"/>
      <!-- 星月头饰 -->
      <text x="31" y="30" font-size="9" fill="#ffd66e" text-anchor="middle">✦</text>
      <text x="70" y="32" font-size="9" fill="#d4c8ff" text-anchor="middle">☽</text>
      <circle cx="68" cy="58" r="1.6" fill="#e8e4ff"/>
      <circle cx="27" cy="62" r="1.2" fill="#e8e4ff" opacity=".7"/>
    </svg>`;
  ...
  function injectDom(opts) {
    ...
    holder.innerHTML = `
      <style>
        ...
        #ap-fab {
          position: fixed; right: 20px; bottom: 96px; z-index: 90; /* 上移避开底部 tab bar */
      ...
      .ap-set {
        ...
        border-radius: 50%; cursor: pointer; color: var(--qa-ink2);
        font-size: 15px; transition: all .25s; margin-right: 4px;
      }
      .ap-set:hover { background: color-mix(in srgb, var(--qa-accent) 15%, transparent); color: var(--qa-accent); transform: rotate(45deg); }
      .llm-badge {
        display: inline-block; vertical-align: middle; margin-left: 6px;
        font-size: 10px; padding: 1px 8px; border-radius: 999px;
        border: 1px solid var(--qa-line); color: var(--qa-ink2);
        font-family: "Avenir Next", sans-serif; letter-spacing: 1px;
      }
      .llm-badge.on { border-color: #7ee090; color: #7ee090; background: rgba(126,224,144,.1); }
      .ap-settings {
        padding: 12px 16px 14px; border-bottom: 1px solid var(--qa-line);
        background: color-mix(in srgb, var(--qa-bg) 80%, transparent);
      }
      .ap-settings .s-title { font-size: 14px; font-weight: 600; color: var(--qa-ink); margin-bottom: 3px; }
      .ap-settings .s-sub { font-size: 11px; color: var(--qa-ink2); margin-bottom: 10px; line-height: 1.6; }
      .ap-settings .s-select, .ap-settings .s-input {
        width: 100%; margin-bottom: 8px; padding: 9px 12px;
        border-radius: 10px; border: 1px solid var(--qa-line);
        background: var(--qa-panel); color: var(--qa-ink); font-size: 13px;
        outline: none;
      }
      .ap-settings .s-select:focus, .ap-settings .s-input:focus { border-color: var(--qa-accent); }
      .ap-settings .s-row { display: flex; gap: 8px; margin-top: 4px; }
      .ap-settings .s-btn {
        flex: 1; padding: 8px 0; border-radius: 999px; cursor: pointer;
        border: 1px solid var(--qa-accent); background: color-mix(in srgb, var(--qa-accent) 18%, transparent);
        color: var(--qa-ink); font-size: 12px; letter-spacing: 2px; transition: all .25s;
      }
      .ap-settings .s-btn:hover { background: color-mix(in srgb, var(--qa-accent) 30%, transparent); }
      .ap-settings .s-btn.ghost { background: transparent; border-color: var(--qa-line); color: var(--qa-ink2); }
      .ap-settings .s-hint { font-size: 10.5px; color: var(--qa-ink2); opacity: .8; margin-top: 8px; line-height: 1.6; }
      /* 加载动画 */
      .qa-loading { display: flex; align-items: center; gap: 8px; color: var(--qa-ink2); font-size: 13px; padding: 8px 2px; }
      .dots { display: inline-flex; gap: 4px; }
      .dots i { width: 6px; height: 6px; border-radius: 50%; background: var(--qa-accent); animation: dotBounce 1.2s infinite; }
      .dots i:nth-child(2) { animation-delay: .15s; }
      .dots i:nth-child(3) { animation-delay: .3s; }
      @keyframes dotBounce { 0%,60%,100% { transform: translateY(0); opacity: .5; } 30% { transform: translateY(-5px); opacity: 1; } }
      .caret { color: var(--qa-accent); animation: blink 0.8s infinite; }
      @keyframes blink { 0%,50% { opacity: 1; } 50.01%,100% { opacity: 0; } }
      .qa-llm { line-height: 1.85; color: var(--qa-ink); font-size: 14px; }
      .qa-llm br { display: block; margin-bottom: 6px; }
      </style>
      ...
      <div id="ap-panel">
        <div id="ap-head">
          ...
          <div class="who">
            <div class="n">${p.name} <span class="llm-badge" id="ap-llm-badge">本地规则</span></div>
            <div class="t">${p.title} · ${SYSTEM_LABEL[state.system]}</div>
          </div>
          <span class="ap-set" id="ap-set" title="AI 大模型设置">⚙</span>
          <span class="x" id="ap-close">✕</span>
        </div>
        <div id="ap-settings" class="ap-settings" style="display:none;">
          <div class="s-title">AI 大模型接入</div>
          <div class="s-sub">用真实大模型让师傅更会解读；未配置时自动使用本地规则。</div>
          <select id="ap-s-provider" class="s-select">
            <option value="deepseek">DeepSeek · 深度求索</option>
            <option value="kimi">Kimi · 月之暗面</option>
            <option value="glm">GLM · 智谱清言</option>
            <option value="openai">OpenAI</option>
          </select>
          <input id="ap-s-key" class="s-input" type="password" placeholder="粘贴你的 API Key" autocomplete="off">
          <div class="s-row">
            <button id="ap-s-save" class="s-btn">保存并启用</button>
            <button id="ap-s-test" class="s-btn ghost">测试连接</button>
            <button id="ap-s-clear" class="s-btn ghost">停用</button>
          </div>
          <div id="ap-s-result" class="s-result"></div>
          <div class="s-hint">Key 只保存在本机浏览器，不会上传。DeepSeek / Kimi / GLM 官网可免费申请。</div>
        </div>
        <div id="ap-body">
          <div id="ap-chips"></div>
          <div id="ap-input-row">
    ...
    /* LLM 初始化：徽章状态 + 预填配置 */
    refreshLlmBadge();
    const savedCfg = llmConfig();
    if (savedCfg && LLM_PROVIDERS[savedCfg.provider]) {
      const sp = document.getElementById("ap-s-provider");
      const sk = document.getElementById("ap-s-key");
      if (sp) sp.value = savedCfg.provider;
      if (sk && savedCfg.key) sk.value = savedCfg.key;
    }
  }
  ...
  function bindEvents() {
    ...
    /* LLM 设置交互 */
    const setBtn = document.getElementById('ap-set');
    const settings = document.getElementById('ap-settings');
    if (setBtn && settings) {
      setBtn.onclick = () => { settings.style.display = settings.style.display === 'none' ? 'block' : 'none'; };
    }
    const sSave = document.getElementById('ap-s-save');
    if (sSave) sSave.onclick = () => {
      const provider = document.getElementById('ap-s-provider').value;
      const key = document.getElementById('ap-s-key').value.trim();
      if (!key) { alert('请先粘贴 API Key。'); return; }
      const model = (LLM_PROVIDERS[provider] || {}).model || '';
      llmSave({ provider: provider, key: key, model: model });
      refreshLlmBadge();
      const s = document.getElementById('ap-settings'); if (s) s.style.display = 'none';
      const ans = document.getElementById('ap-answer');
      const cfg = llmConfig();
      ans.innerHTML = '<div class="qa-item"><div class="qq">已接入</div>' +
        '师傅已连上 <b>' + LLM_PROVIDERS[cfg.provider].label + '</b>（' + (cfg.model || '默认模型') + '）。<br>' +
        '现在提问，师傅会调用大模型结合你的盘面详细解读。</div>';
    };
    /* 测试连接：立即验证 Key 是否有效 */
    const sTest = document.getElementById('ap-s-test');
    if (sTest) sTest.onclick = () => {
      const provider = document.getElementById('ap-s-provider').value;
      const key = document.getElementById('ap-s-key').value.trim();
      const resEl = document.getElementById('ap-s-result');
      if (!key) { resEl.className = 's-result bad'; resEl.textContent = '✗ 请先粘贴 API Key。'; return; }
      const model = (LLM_PROVIDERS[provider] || {}).model || '';
      const tCfg = { provider: provider, key: key, model: model };
      resEl.className = 's-result busy';
      resEl.textContent = '⏳ 正在连接 ' + LLM_PROVIDERS[provider].label + ' …';
      const tMessages = [{ role: 'user', content: '你好，请回复"连接成功"四个字。' }];
      // 复用 callLLM（它内部读 llmConfig，这里临时换配置再还原）
      const prev = llmConfig();
      llmSave(tCfg);
      callLLM(tMessages).then(function(text) {
        resEl.className = 's-result ok';
        resEl.textContent = '✓ 连接成功！模型已回复：' + (text.length > 40 ? text.slice(0, 40) + '…' : text);
      }).catch(function(err) {
        const msg = String((err && err.message) || err);
        let hint = '';
        if (msg.indexOf('401') >= 0) hint = 'API Key 无效（HTTP 401）——请检查 Key 是否正确、是否复制全、是否选了对应的服务商。';
        else if (msg.indexOf('402') >= 0) hint = '账户余额不足（HTTP 402），需要充值。';
        else if (msg.indexOf('403') >= 0) hint = '被拒绝访问（HTTP 403）——检查 Key 权限或服务商限制。';
        else if (msg.indexOf('429') >= 0) hint = '请求太频繁（HTTP 429），稍等几秒再试。';
        else if (msg.indexOf('Failed to fetch') >= 0 || msg.indexOf('NetworkError') >= 0) hint = '网络无法到达服务商（Failed to fetch）——检查网络/代理，或换一个服务商试试。';
        else if (msg.indexOf('空回复') >= 0) hint = '模型返回空内容，可换一个模型试试。';
        else hint = msg;
        resEl.className = 's-result bad';
        resEl.textContent = '✗ 连接失败：' + hint;
      }).then(function() {
        if (prev) llmSave(prev); else llmClear();
      });
    };
    const sClear = document.getElementById('ap-s-clear');
    if (sClear) sClear.onclick = () => {
      llmClear();
      document.getElementById('ap-s-key').value = '';
      refreshLlmBadge();
      const s = document.getElementById('ap-settings'); if (s) s.style.display = 'none';
      const ans = document.getElementById('ap-answer');
      ans.innerHTML = '<div class="qa-item"><div class="qq">已停用</div>师傅回到本地规则解读模式。随时可以在 ⚙ 里重新接入大模型。</div>';
    };
  ...
  /* ============================================================
     LLM 大模型接入（DeepSeek / Kimi / GLM / OpenAI，OpenAI 兼容接口）
     未配置 Key 时自动降级到本地规则解读
     ============================================================ */
  const LLM_PROVIDERS = {
    deepseek: { label: 'DeepSeek · 深度求索', endpoint: 'https://api.deepseek.com/v1/chat/completions', model: 'deepseek-chat', ph: 'sk-…' },
    kimi:     { label: 'Kimi · 月之暗面',     endpoint: 'https://api.moonshot.cn/v1/chat/completions', model: 'moonshot-v1-8k', ph: 'sk-…' },
    glm:      { label: 'GLM · 智谱清言',      endpoint: 'https://open.bigmodel.cn/api/paas/v4/chat/completions', model: 'glm-4-flash', ph: '你的智谱 API Key' },
    openai:   { label: 'OpenAI',              endpoint: 'https://api.openai.com/v1/chat/completions', model: 'gpt-4o-mini', ph: 'sk-…' },
  };
  function llmConfig() { try { return JSON.parse(localStorage.getItem('kixs_llm') || 'null'); } catch(e) { return null; } }
  function llmSave(cfg) { try { localStorage.setItem('kixs_llm', JSON.stringify(cfg)); } catch(e) {} }
  function llmClear() { try { localStorage.removeItem('kixs_llm'); } catch(e) {} }
  function llmReady() { const c = llmConfig(); return !!(c && c.key && LLM_PROVIDERS[c.provider]); }

  /* 命盘 → 给 LLM 的详细摘要 */
  function llmChartSummary(data) {
    if (!data) return '';
    const r = data.result, draws = data.draws, ch = data.chart;
    const parts = [];
    if (r && r.benBits) parts.push('六爻：本卦' + (r.ben && r.ben.name || '') + (r.bian && r.bian.name ? '，变卦' + r.bian.name : '') + '，动爻' + (r.movingCount || 0) + '个' + (r.movingPos && r.movingPos.length ? '（' + r.movingPos.join('、') + '爻）' : ''));
    else if (r && r.ben && r.ben.name && r.ti && r.yong) parts.push('梅花易数：本卦' + r.ben.name + '，互卦' + (r.hu ? r.hu.name : '-') + '，变卦' + (r.bian ? r.bian.name : '-') + '，体卦' + r.ti.name + '，用卦' + r.yong.name + '，体用' + r.relation + '，' + r.level);
    if (r && r.dayGanzhi) parts.push('大六壬：日' + r.dayGanzhi + '，三传' + r.sanChuan.join('→') + '，' + r.keType + '，' + (r.sanQin ? r.sanQin.join('、') : ''));

    if (draws && draws.length) parts.push('塔罗：' + draws.map(function(d){ return d.position.name + '位·' + d.name + (d.reversed ? '逆' : '正'); }).join('；'));
    if (ch && ch.planets) {
      var sun = ch.planets.find(function(p){ return p.key === 'sun'; });
      var moon = ch.planets.find(function(p){ return p.key === 'moon'; });
      var venus = ch.planets.find(function(p){ return p.key === 'venus'; });
      parts.push('星盘：太阳' + (sun ? sun.sign.name + '(' + sun.house + '宫)' : '') + '，月亮' + (moon ? moon.sign.name + '(' + moon.house + '宫)' : '') + '，金星' + (venus ? venus.sign.name + '(' + venus.house + '宫)' : '') + '，上升' + (ch.asc ? ch.asc.sign.name : ''));
    }
    if (r && r.pillars) parts.push('八字：四柱' + r.pillars.map(function(p){ return p.gan + p.zhi; }).join(' ') + '，日主' + r.dayMaster + r.dayMasterWu + (r.wuCount ? '，五行' + Object.keys(r.wuCount).map(function(k){ return k + r.wuCount[k]; }).join(' ') : ''));
    if (ch && ch.palaces) parts.push('紫微：命宫' + (ch.palaces[0] && ch.palaces[0].stars ? ch.palaces[0].stars.join('、') : '') + '，十二宫已排');
    return parts.join('\n');
  }

  /* 师傅人设（system prompt） */
  function llmSystemPrompt() {
    var p = personaOf(state.system);
    var isWest = p.name === '月见';
    return (isWest
      ? '你是月见，一位温柔细腻的女性占星师与塔罗师，说话轻声、善用比喻和画面感，擅长从星盘与牌面看情绪、关系与时机。'
      : '你是玄机子，一位沉稳博学的男性玄学大师，精通八字、紫微、梅花、六爻、大六壬，说话简练有层次，直指要害。')
      + '你正在为用户解读命盘/卦象。规则：①必须结合下方给出的盘面信息具体解读，不要泛泛而谈；②结论清晰不和稀泥，但禁止恐吓式、绝对化断言（如"必定离婚""血光之灾"）；③命理是象征推演，最后轻轻提示仅供参考；④用简体中文，回答 400-600 字，分 3-5 小段，每段独立一行，段落间空行，不要用 markdown 标题符号。';
  }

  /* 调用 LLM（OpenAI 兼容接口） */
  function callLLM(messages) {
    var c = llmConfig();
    var p = LLM_PROVIDERS[c.provider];
    return fetch(p.endpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + c.key },
      body: JSON.stringify({ model: c.model || p.model, messages: messages, temperature: 0.7, max_tokens: 900, stream: false })
    }).then(function(res) {
  ...
  function typewriter(el, text) {
    ...
    var timer = setInterval(function() {
      i += 4;
      el.innerHTML = html.slice(0, i) + (i < html.length ? '<span class="caret">▍</span>' : '');
  ...
  function refreshLlmBadge() {
    var b = document.getElementById('ap-llm-badge');
    if (!b) return;
    var cfg = llmConfig();
    if (llmReady()) {
      b.textContent = 'AI · ' + LLM_PROVIDERS[cfg.provider].label.split(' ')[0];
      b.className = 'llm-badge on';
    } else {
      b.textContent = '本地规则';
  ...
  function ask(q) {
    ...
    /* ===== LLM 模式（已配置 API Key） ===== */
    if (llmReady()) {
      ans.innerHTML = '<div class="qa-item"><div class="qq">❖ ' + q + '</div>' +
        '<div class="qa-loading"><span class="dots"><i></i><i></i><i></i></span> ' + p.name + ' 正在结合你的盘解读…</div></div>';
      const body0 = document.getElementById('ap-body');
      if (body0) body0.scrollTop = body0.scrollHeight;
      const kbCtx = KB ? KB.search(q, state.system) : null;
      const chartCtx = llmChartSummary(state.chart);
      const sysMsg = { role: 'system', content: llmSystemPrompt() };
      const userMsg = { role: 'user', content:
        (chartCtx ? '【命盘信息】\n' + chartCtx + '\n\n' : '') +
        (kbCtx ? '【参考知识】\n' + kbCtx + '\n\n' : '') +
        '【用户提问】\n' + q };
      callLLM([sysMsg, userMsg]).then(function(text) {
        const qa = document.createElement('div');
        qa.className = 'qa-item';
        qa.innerHTML = '<div class="qq">' + p.name + ' 解读</div><div class="qa-llm"></div>';
        const load = ans.querySelector('.qa-loading');
        if (load && load.parentElement) load.parentElement.replaceWith(qa);
        else ans.appendChild(qa);
        typewriter(qa.querySelector('.qa-llm'), text);
      }).catch(function(err) {
        ...
        fb.className = 'qa-item';
        fb.innerHTML = '<div class="qq">' + p.name + ' 解读</div>' +
          '<div class="qa-fallback"><b>AI 连接失败（' + String((err && err.message) || err).slice(0, 60) + '）</b>，已改用本地解读：<br>' +
          final.replace(/\n/g, '<br>') + '</div>';
        const load2 = ans.querySelector('.qa-loading');
        if (load2 && load2.parentElement) load2.parentElement.replaceWith(fb);
        else ans.appendChild(fb);
        const body2 = document.getElementById('ap-body');
    ...
    const chain = thinkChain(q, state.system, area);
    const final = (global.ToneEngine && chain.negative) ? global.ToneEngine.soften(chain.text, true) : chain.text;
    ans.innerHTML = '<div class="qa-think" id="ap-think">' +
